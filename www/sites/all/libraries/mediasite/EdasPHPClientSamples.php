...
//    error_reporting(E_ERROR | E_WARNING | E_PARSE);
	...
	$render_params = array(
		...
		"svc_url" => isset($_SESSION['svc_url']) ? $_SESSION['svc_url']: "http://Your_Mediasite_Root/Services60/Edas.svc",
			...
			if(get_headers($_POST['svc_url'])) {
				...
				try {
					...
					$login_resp = $client->Login($_POST['username'], $_POST['pass'], $_SESSION['application_name'],
							$_SESSION['impersonation_username'] );
	...
	*/
	function QueryPresentations(&$render_params) {
		...
		try {
			$client = new ExternalAccessClient($_SESSION['svc_url']);
			$result = $client->QueryPresentationsByCriteria($criteria, null, $_SESSION['userticket']);
		} catch (Exception $ex) {
			throw $ex;
		}
